...
pub fn register_user(m: &Mn, email: &str, name: &str, password: &str) -> FResult<i32> {
    ...
    let password = amitu_base::hash_password(password)?;
    ...
    m.conn.transaction(|| {
        let uid: i32 = diesel::insert_into(teja_user::table)
            .values((
                ...
                teja_user::created_on.eq(m.now),
                teja_user::updated_on.eq(m.now),
            ))
